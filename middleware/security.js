...
const validateRequest = (req, res, next) => {
  ...
  if (req.method === "POST") {
    ...
    // Allow multipart/form-data for image uploads
  ...
  // Check request size (allow larger size for image uploads)
  const contentLength = parseInt(req.headers["content-length"] || "0");
  const maxSize =
    req.path.includes("/upload-image") || req.path.includes("/upload-photo")
      ? 10 * 1024 * 1024
      : 1024 * 1024; // 10MB for uploads, 1MB for others
